...
const App = {
    ...
    async init() {
        console.log('Inicializace PRAUT Průvodce - Phase 4...');
    ...
    checkFirstVisit() {
        const hasVisited = localStorage.getItem('phase4-visited');
        if (!hasVisited) {
            this.showOnboarding();
        }
    },
        ...
        overlay.innerHTML = `
            <div class="onboarding-modal">
                <div class="onboarding-header">
                    ...
                    <h2>Vítejte v God Mode!</h2>
                </div>
                <div class="onboarding-content">
                    <p>Toto je pokročilá úroveň pro power usery. Naučíte se maximálně využít AI agenty.</p>

                    <div class="onboarding-tips">
                        <div class="onboarding-tip">
                            <span class="tip-icon">🤖</span>
                            <div>
                                <strong>Multi-Agent Orchestrace</strong>
                                <p>Koordinujte více AI agentů současně</p>
                            </div>
                        </div>
                        <div class="onboarding-tip">
                            <span class="tip-icon">🔌</span>
                            <div>
                                <strong>MCP Integrace</strong>
                                <p>Propojte agenty s externími nástroji</p>
                            </div>
                        </div>
                        <div class="onboarding-tip">
                            <span class="tip-icon">⚡</span>
                            <div>
                                <strong>Vlastní Agenti</strong>
                                <p>Vytvářejte specializované AI asistenty</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    closeOnboarding() {
        ...
        localStorage.setItem('phase4-visited', 'true');
    },
    ...
    loadTheme() {
        const saved = localStorage.getItem('phase4-theme');
    ...
    setTheme(theme) {
        ...
        localStorage.setItem('phase4-theme', theme);
    ...
    saveProgress(sectionId, isNew = false) {
        ...
        let progress = JSON.parse(localStorage.getItem('phase4-progress') || '[]');
        if (!progress.includes(sectionId)) {
            progress.push(sectionId);
            localStorage.setItem('phase4-progress', JSON.stringify(progress));
    ...
    loadProgress() {
        const progress = JSON.parse(localStorage.getItem('phase4-progress') || '[]');
        this.currentState.completedSections = new Set(progress);
        this.updateProgressUI();
