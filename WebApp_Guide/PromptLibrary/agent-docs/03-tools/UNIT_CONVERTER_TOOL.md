---
description: >-
  Unit Converter Tool - Production-Ready Tool Specification.
...
# 🛠️ Unit Converter Tool

> **Capability**: Allows the agent to perform **unit_converter_tool** operations securely and efficiently.
...
```json
{
  ...
  "description": "Executes unit_converter_tool operations with strict validation.",
  "parameters": {
    "type": "object",
    "properties": {
      "target_resource": {
...
- **Authentication**: Usage requires Validated Bearer Token (Scope: `unit_converter_tool:execute`).
...
class UnitConverterToolInput(BaseModel):
    target: str = Field(..., description="Target Endpoint")
    params: Dict[str, Any] = Field(default_factory=dict)

def execute_unit_converter_tool(input_data: UnitConverterToolInput) -> str:
    """
    Implements the core logic for Unit Converter Tool.
    """
