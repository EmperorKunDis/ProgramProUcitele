---
description: >-
  Digitalocean Tool - Production-Ready Tool Specification.
...
> **Capability**: Allows the agent to perform **digitalocean_tool** operations securely and efficiently.
...
```json
{
  "name": "digitalocean_tool",
  "description": "Executes digitalocean_tool operations with strict validation.",
  "parameters": {
    "type": "object",
    "properties": {
      "target_resource": {
...
- **Authentication**: Usage requires Validated Bearer Token (Scope: `digitalocean_tool:execute`).
...
class DigitaloceanToolInput(BaseModel):
    target: str = Field(..., description="Target Endpoint")
    params: Dict[str, Any] = Field(default_factory=dict)

def execute_digitalocean_tool(input_data: DigitaloceanToolInput) -> str:
    """
    Implements the core logic for Digitalocean Tool.
    """
