---
description: >-
  Rss Reader Tool - Production-Ready Tool Specification.
...
> **Capability**: Allows the agent to perform **rss_reader_tool** operations securely and efficiently.
...
```json
{
  "name": "rss_reader_tool",
  "description": "Executes rss_reader_tool operations with strict validation.",
  "parameters": {
    "type": "object",
    "properties": {
      "target_resource": {
...
- **Authentication**: Usage requires Validated Bearer Token (Scope: `rss_reader_tool:execute`).
...
class RssReaderToolInput(BaseModel):
    target: str = Field(..., description="Target Endpoint")
    params: Dict[str, Any] = Field(default_factory=dict)

def execute_rss_reader_tool(input_data: RssReaderToolInput) -> str:
    """
    Implements the core logic for Rss Reader Tool.
    """
