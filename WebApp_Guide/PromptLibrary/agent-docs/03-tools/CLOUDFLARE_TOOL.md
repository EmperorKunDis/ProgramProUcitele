---
description: >-
  Cloudflare Tool - Production-Ready Tool Specification.
...
> **Capability**: Allows the agent to perform **cloudflare_tool** operations securely and efficiently.
...
```json
{
  "name": "cloudflare_tool",
  "description": "Executes cloudflare_tool operations with strict validation.",
  "parameters": {
    "type": "object",
    "properties": {
      "target_resource": {
...
- **Authentication**: Usage requires Validated Bearer Token (Scope: `cloudflare_tool:execute`).
...
class CloudflareToolInput(BaseModel):
    target: str = Field(..., description="Target Endpoint")
    params: Dict[str, Any] = Field(default_factory=dict)

def execute_cloudflare_tool(input_data: CloudflareToolInput) -> str:
    """
    Implements the core logic for Cloudflare Tool.
    """
