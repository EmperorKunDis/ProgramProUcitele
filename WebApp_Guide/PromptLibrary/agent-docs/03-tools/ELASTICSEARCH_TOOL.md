---
description: >-
  Elasticsearch Tool - Production-Ready Tool Specification.
...
# 🛠️ Elasticsearch Tool

> **Capability**: Allows the agent to perform **elasticsearch_tool** operations securely and efficiently.
...
```json
{
  ...
  "description": "Executes elasticsearch_tool operations with strict validation.",
  "parameters": {
    "type": "object",
    "properties": {
      "target_resource": {
...
- **Authentication**: Usage requires Validated Bearer Token (Scope: `elasticsearch_tool:execute`).
...
class ElasticsearchToolInput(BaseModel):
    target: str = Field(..., description="Target Endpoint")
    params: Dict[str, Any] = Field(default_factory=dict)

def execute_elasticsearch_tool(input_data: ElasticsearchToolInput) -> str:
    """
    Implements the core logic for Elasticsearch Tool.
    """
