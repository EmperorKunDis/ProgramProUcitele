---
description: >-
  Postgresql Tool - Production-Ready Tool Specification.
...
> **Capability**: Allows the agent to perform **postgresql_tool** operations securely and efficiently.
...
```json
{
  "name": "postgresql_tool",
  "description": "Executes postgresql_tool operations with strict validation.",
  "parameters": {
    "type": "object",
    "properties": {
      "target_resource": {
...
- **Authentication**: Usage requires Validated Bearer Token (Scope: `postgresql_tool:execute`).
...
class PostgresqlToolInput(BaseModel):
    target: str = Field(..., description="Target Endpoint")
    params: Dict[str, Any] = Field(default_factory=dict)

def execute_postgresql_tool(input_data: PostgresqlToolInput) -> str:
    """
    Implements the core logic for Postgresql Tool.
    """
