---
description: >-
  Sse Integration - Production-Ready Tool Specification.
...
> **Capability**: Allows the agent to perform **sse_integration** operations securely and efficiently.
...
```json
{
  ...
  "description": "Executes sse_integration operations with strict validation.",
  "parameters": {
    "type": "object",
    "properties": {
      "target_resource": {
...
- **Authentication**: Usage requires Validated Bearer Token (Scope: `sse_integration:execute`).
...
class SseIntegrationInput(BaseModel):
    target: str = Field(..., description="Target Endpoint")
    params: Dict[str, Any] = Field(default_factory=dict)

def execute_sse_integration(input_data: SseIntegrationInput) -> str:
    """
    Implements the core logic for Sse Integration.
    """
