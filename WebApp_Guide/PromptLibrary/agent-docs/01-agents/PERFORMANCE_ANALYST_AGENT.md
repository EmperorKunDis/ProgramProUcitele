---
title: Performance Analyst Agent
...
### 1.1 Purpose
The **Performance Analyst Agent** represents a cornerstone capability within the autonomous agent grid. Its primary directive is to assume full control over **performance_analyst_agent** operations, providing a deterministic, auditable, and scalable interface for upstream orchestrators.

In modern multi-agent architectures, "narrow AI" components like this one must be excessively defined to prevent "semantic drift" — the phenomenon where an agent's behavior deviates from its original prompt instructions over long-running sessions. This document serves as the **Ground Truth** for the behavior, constraints, and capabilities of the Performance Analyst Agent.
...
### 1.3 Strategic Value
By implementing the Performance Analyst Agent, the organization gains:
1. **Reduced Cognitive Load**: Offloading performance_analyst_agent tasks to a specialized solver.
2. **Standardized Output**: Enforcing a strict schema for all deliverables.
3. **Traceability**: Complete provenance of every decision made during performance_analyst_agent execution.
...
### 2.1 Core Philosophy
The architecture of Performance Analyst Agent follows the **Hexagonal Architecture** (Ports and Adapters) pattern to ensure complete decoupling of the core logic from external tools and databases.
...
graph TB
    ...
    subgraph "Performance Analyst Agent Boundary"
        InputAdapter[Input Adapter]
        CoreLogic[**Performance Analyst Agent Core**]
...
This is the "Brain" of the Performance Analyst Agent. It utilizes a **ReAct** (Reasoning + Acting) loop modified with a **Reflection** step.
...
Connects to the vector database (e.g., Pinecone/Weaviate) to retrieve relevant "Few-Shot" examples dynamic to the current context. This allows the Performance Analyst Agent to "remember" past mistakes and avoid repeating them.


#### 2.3.4 Sub-Module B
Detailed specification for internal module B. This module handles edge cases concerning performance_analyst_agent specific logic. It implements the standard `IComponent` interface and provides telemetry hooks.
- **Input**: Stream<Bytes>
- **Output**: Stream<JSON>
- **Latency Budget**: < 50ms

#### 2.3.5 Sub-Module C
Detailed specification for internal module C. This module handles edge cases concerning performance_analyst_agent specific logic. It implements the standard `IComponent` interface and provides telemetry hooks.
- **Input**: Stream<Bytes>
- **Output**: Stream<JSON>
- **Latency Budget**: < 50ms

#### 2.3.6 Sub-Module D
Detailed specification for internal module D. This module handles edge cases concerning performance_analyst_agent specific logic. It implements the standard `IComponent` interface and provides telemetry hooks.
- **Input**: Stream<Bytes>
- **Output**: Stream<JSON>
- **Latency Budget**: < 50ms

#### 2.3.7 Sub-Module E
Detailed specification for internal module E. This module handles edge cases concerning performance_analyst_agent specific logic. It implements the standard `IComponent` interface and provides telemetry hooks.
- **Input**: Stream<Bytes>
- **Output**: Stream<JSON>
- **Latency Budget**: < 50ms

#### 2.3.8 Sub-Module F
Detailed specification for internal module F. This module handles edge cases concerning performance_analyst_agent specific logic. It implements the standard `IComponent` interface and provides telemetry hooks.
- **Input**: Stream<Bytes>
- **Output**: Stream<JSON>
- **Latency Budget**: < 50ms
...
|---------------|----------------------|------|---------|-------------|--------------|
| `core.timeout` | `CORE_TIMEOUT` | Float | `1.0` | Controls the Timeout behavior for the core subsystem. Tuning this affects performance. | High |
| `core.retrycount` | `CORE_RETRYCOUNT` | Boolean | `1.0` | Controls the RetryCount behavior for the core subsystem. Tuning this affects performance. | High |
| `core.temperature` | `CORE_TEMPERATURE` | Integer | `True` | Controls the Temperature behavior for the core subsystem. Tuning this affects performance. | High |
| `core.topp` | `CORE_TOPP` | String | `/tmp` | Controls the TopP behavior for the core subsystem. Tuning this affects performance. | High |
| `core.contextwindow` | `CORE_CONTEXTWINDOW` | Float | `3000` | Controls the ContextWindow behavior for the core subsystem. Tuning this affects performance. | High |
| `core.memorykey` | `CORE_MEMORYKEY` | Integer | `3000` | Controls the MemoryKey behavior for the core subsystem. Tuning this affects performance. | High |
| `core.logpath` | `CORE_LOGPATH` | String | `1.0` | Controls the LogPath behavior for the core subsystem. Tuning this affects performance. | High |
| `core.cachettl` | `CORE_CACHETTL` | Float | `1.0` | Controls the CacheTTL behavior for the core subsystem. Tuning this affects performance. | High |
| `core.maxtokens` | `CORE_MAXTOKENS` | Boolean | `1.0` | Controls the MaxTokens behavior for the core subsystem. Tuning this affects performance. | High |
| `core.stopsequences` | `CORE_STOPSEQUENCES` | Float | `True` | Controls the StopSequences behavior for the core subsystem. Tuning this affects performance. | High |
| `network.timeout` | `NETWORK_TIMEOUT` | Integer | `True` | Controls the Timeout behavior for the network subsystem. Tuning this affects performance. | High |
| `network.retrycount` | `NETWORK_RETRYCOUNT` | Integer | `/tmp` | Controls the RetryCount behavior for the network subsystem. Tuning this affects performance. | High |
| `network.temperature` | `NETWORK_TEMPERATURE` | Float | `1.0` | Controls the Temperature behavior for the network subsystem. Tuning this affects performance. | High |
| `network.topp` | `NETWORK_TOPP` | Boolean | `None` | Controls the TopP behavior for the network subsystem. Tuning this affects performance. | High |
| `network.contextwindow` | `NETWORK_CONTEXTWINDOW` | Float | `None` | Controls the ContextWindow behavior for the network subsystem. Tuning this affects performance. | High |
| `network.memorykey` | `NETWORK_MEMORYKEY` | String | `/tmp` | Controls the MemoryKey behavior for the network subsystem. Tuning this affects performance. | High |
| `network.logpath` | `NETWORK_LOGPATH` | Boolean | `0` | Controls the LogPath behavior for the network subsystem. Tuning this affects performance. | High |
| `network.cachettl` | `NETWORK_CACHETTL` | Boolean | `0` | Controls the CacheTTL behavior for the network subsystem. Tuning this affects performance. | High |
| `network.maxtokens` | `NETWORK_MAXTOKENS` | Integer | `True` | Controls the MaxTokens behavior for the network subsystem. Tuning this affects performance. | High |
| `network.stopsequences` | `NETWORK_STOPSEQUENCES` | Boolean | `3000` | Controls the StopSequences behavior for the network subsystem. Tuning this affects performance. | High |
| `db.timeout` | `DB_TIMEOUT` | Float | `0` | Controls the Timeout behavior for the db subsystem. Tuning this affects performance. | High |
| `db.retrycount` | `DB_RETRYCOUNT` | Integer | `1.0` | Controls the RetryCount behavior for the db subsystem. Tuning this affects performance. | High |
| `db.temperature` | `DB_TEMPERATURE` | Float | `True` | Controls the Temperature behavior for the db subsystem. Tuning this affects performance. | High |
| `db.topp` | `DB_TOPP` | Integer | `0` | Controls the TopP behavior for the db subsystem. Tuning this affects performance. | High |
| `db.contextwindow` | `DB_CONTEXTWINDOW` | Integer | `0` | Controls the ContextWindow behavior for the db subsystem. Tuning this affects performance. | High |
| `db.memorykey` | `DB_MEMORYKEY` | Boolean | `None` | Controls the MemoryKey behavior for the db subsystem. Tuning this affects performance. | High |
| `db.logpath` | `DB_LOGPATH` | String | `/tmp` | Controls the LogPath behavior for the db subsystem. Tuning this affects performance. | High |
| `db.cachettl` | `DB_CACHETTL` | Boolean | `1.0` | Controls the CacheTTL behavior for the db subsystem. Tuning this affects performance. | High |
| `db.maxtokens` | `DB_MAXTOKENS` | Integer | `/tmp` | Controls the MaxTokens behavior for the db subsystem. Tuning this affects performance. | High |
| `db.stopsequences` | `DB_STOPSEQUENCES` | Float | `None` | Controls the StopSequences behavior for the db subsystem. Tuning this affects performance. | High |
| `auth.timeout` | `AUTH_TIMEOUT` | String | `/tmp` | Controls the Timeout behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.retrycount` | `AUTH_RETRYCOUNT` | String | `True` | Controls the RetryCount behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.temperature` | `AUTH_TEMPERATURE` | String | `1.0` | Controls the Temperature behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.topp` | `AUTH_TOPP` | Boolean | `None` | Controls the TopP behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.contextwindow` | `AUTH_CONTEXTWINDOW` | String | `None` | Controls the ContextWindow behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.memorykey` | `AUTH_MEMORYKEY` | Float | `/tmp` | Controls the MemoryKey behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.logpath` | `AUTH_LOGPATH` | Boolean | `3000` | Controls the LogPath behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.cachettl` | `AUTH_CACHETTL` | String | `True` | Controls the CacheTTL behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.maxtokens` | `AUTH_MAXTOKENS` | Integer | `1.0` | Controls the MaxTokens behavior for the auth subsystem. Tuning this affects performance. | High |
| `auth.stopsequences` | `AUTH_STOPSEQUENCES` | String | `None` | Controls the StopSequences behavior for the auth subsystem. Tuning this affects performance. | High |
| `plugin.timeout` | `PLUGIN_TIMEOUT` | Float | `0` | Controls the Timeout behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.retrycount` | `PLUGIN_RETRYCOUNT` | String | `/tmp` | Controls the RetryCount behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.temperature` | `PLUGIN_TEMPERATURE` | Boolean | `True` | Controls the Temperature behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.topp` | `PLUGIN_TOPP` | String | `None` | Controls the TopP behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.contextwindow` | `PLUGIN_CONTEXTWINDOW` | Integer | `True` | Controls the ContextWindow behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.memorykey` | `PLUGIN_MEMORYKEY` | Integer | `0` | Controls the MemoryKey behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.logpath` | `PLUGIN_LOGPATH` | Integer | `None` | Controls the LogPath behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.cachettl` | `PLUGIN_CACHETTL` | Float | `1.0` | Controls the CacheTTL behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.maxtokens` | `PLUGIN_MAXTOKENS` | Float | `1.0` | Controls the MaxTokens behavior for the plugin subsystem. Tuning this affects performance. | High |
| `plugin.stopsequences` | `PLUGIN_STOPSEQUENCES` | Float | `1.0` | Controls the StopSequences behavior for the plugin subsystem. Tuning this affects performance. | High |
| `ui.timeout` | `UI_TIMEOUT` | Boolean | `3000` | Controls the Timeout behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.retrycount` | `UI_RETRYCOUNT` | String | `None` | Controls the RetryCount behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.temperature` | `UI_TEMPERATURE` | Boolean | `/tmp` | Controls the Temperature behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.topp` | `UI_TOPP` | String | `/tmp` | Controls the TopP behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.contextwindow` | `UI_CONTEXTWINDOW` | Boolean | `True` | Controls the ContextWindow behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.memorykey` | `UI_MEMORYKEY` | Integer | `0` | Controls the MemoryKey behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.logpath` | `UI_LOGPATH` | String | `1.0` | Controls the LogPath behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.cachettl` | `UI_CACHETTL` | Float | `/tmp` | Controls the CacheTTL behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.maxtokens` | `UI_MAXTOKENS` | String | `1.0` | Controls the MaxTokens behavior for the ui subsystem. Tuning this affects performance. | High |
| `ui.stopsequences` | `UI_STOPSEQUENCES` | Integer | `1.0` | Controls the StopSequences behavior for the ui subsystem. Tuning this affects performance. | High |
| `audit.timeout` | `AUDIT_TIMEOUT` | Boolean | `1.0` | Controls the Timeout behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.retrycount` | `AUDIT_RETRYCOUNT` | Boolean | `0` | Controls the RetryCount behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.temperature` | `AUDIT_TEMPERATURE` | Float | `3000` | Controls the Temperature behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.topp` | `AUDIT_TOPP` | Float | `1.0` | Controls the TopP behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.contextwindow` | `AUDIT_CONTEXTWINDOW` | Integer | `0` | Controls the ContextWindow behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.memorykey` | `AUDIT_MEMORYKEY` | Float | `0` | Controls the MemoryKey behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.logpath` | `AUDIT_LOGPATH` | Float | `/tmp` | Controls the LogPath behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.cachettl` | `AUDIT_CACHETTL` | Integer | `True` | Controls the CacheTTL behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.maxtokens` | `AUDIT_MAXTOKENS` | String | `None` | Controls the MaxTokens behavior for the audit subsystem. Tuning this affects performance. | High |
| `audit.stopsequences` | `AUDIT_STOPSEQUENCES` | Float | `3000` | Controls the StopSequences behavior for the audit subsystem. Tuning this affects performance. | High |
| `experimental.timeout` | `EXPERIMENTAL_TIMEOUT` | Boolean | `/tmp` | Controls the Timeout behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.retrycount` | `EXPERIMENTAL_RETRYCOUNT` | Float | `None` | Controls the RetryCount behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.temperature` | `EXPERIMENTAL_TEMPERATURE` | Float | `True` | Controls the Temperature behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.topp` | `EXPERIMENTAL_TOPP` | Boolean | `3000` | Controls the TopP behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.contextwindow` | `EXPERIMENTAL_CONTEXTWINDOW` | Float | `True` | Controls the ContextWindow behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.memorykey` | `EXPERIMENTAL_MEMORYKEY` | String | `None` | Controls the MemoryKey behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.logpath` | `EXPERIMENTAL_LOGPATH` | Float | `3000` | Controls the LogPath behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.cachettl` | `EXPERIMENTAL_CACHETTL` | Float | `/tmp` | Controls the CacheTTL behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.maxtokens` | `EXPERIMENTAL_MAXTOKENS` | Boolean | `3000` | Controls the MaxTokens behavior for the experimental subsystem. Tuning this affects performance. | High |
| `experimental.stopsequences` | `EXPERIMENTAL_STOPSEQUENCES` | Float | `0` | Controls the StopSequences behavior for the experimental subsystem. Tuning this affects performance. | High |
...
The primary interaction with the Performance Analyst Agent is via the `AgentRunner` class.

```python
from typing import List, Dict, Optional, Union
from pydantic import BaseModel

class Performance_analyst_agentResult(BaseModel):
    success: bool
    data: Dict[str, Any]
    metrics: Dict[str, float]
    trace_id: str

class Performance_analyst_agentRunner:
    ...
    async def run_task(
    ...
    ) -> Performance_analyst_agentResult:
        """
        ...
        Returns:
            A structured Performance_analyst_agentResult object.
...
To deploy the **Performance Analyst Agent** in a production Kubernetes environment, follow these strict procedures.
...
### 7.1 Data Protection
All data traversing the Performance Analyst Agent is encrypted at rest and in transit using TLS 1.3...
