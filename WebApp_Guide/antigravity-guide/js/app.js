...
const App = {
    ...
    checkFirstVisit() {
        const hasVisited = localStorage.getItem('antigravity-visited');
        if (!hasVisited) {
            this.showOnboarding();
        }
    },
    ...
    closeOnboarding() {
        ...
        localStorage.setItem('antigravity-visited', 'true');
    },
    ...
    saveProgress(sectionId, isNew = false) {
        ...
        let progress = JSON.parse(localStorage.getItem('antigravity-progress') || '[]');
        if (!progress.includes(sectionId)) {
            progress.push(sectionId);
            localStorage.setItem('antigravity-progress', JSON.stringify(progress));
    ...
    loadProgress() {
        const progress = JSON.parse(localStorage.getItem('antigravity-progress') || '[]');
        this.currentState.completedSections = new Set(progress);
        this.updateProgressUI();
